                        ...
                        @if($content->content->mode == 'publish')
                            ...
                            <div class="owl-car-s" dir="rtl">
                                <a href="/product/{{ $popular->id }}/{!! \Illuminate\Support\Str::slug($popular->title) ?? '-' !!}" title="{{ $popular->title }}" class="content-box">
                                    ...
                                    <div class="footer">
                                        <span class="avatar" title="{{ isset($popular->user->name)?? $popular->user->name }}" onclick="window.location.href = '/profile/{{ isset($popular->user->id) ?? $popular->user->id }}'"><img src="{{ get_user_meta(isset($popular->user->id) ?? $popular->user->id,'avatar',get_option('default_user_avatar','')) }}"></span>
